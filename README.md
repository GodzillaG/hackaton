...
Сценарий работы: ноутбук запускает сервер анализа и Next.js сайт, телефон находится в той же Wi-Fi сети, открывает сайт по IP ноутбука, входит в аккаунт, проходит протокол съёмки из пяти ракурсов и получает отчёт с уровнем риска, метриками, планом действий и изображениями с разметкой.
...
Также создаётся аккаунт `admin` с паролем `12345678`. Старые файлы из корневой папки `reports/` мигрируются в SQLite под этим аккаунтом.
