...
Сценарий работы: ноутбук запускает сервер анализа и Next.js сайт, телефон находится в той же Wi-Fi сети, открывает сайт по IP ноутбука, отправляет фото ученика и получает отчёт с уровнем риска, метриками, рекомендациями и изображением с разметкой.
...
- веб-результат: индикатор риска, карточки метрик, рекомендации;
- overlay-картинка с разметкой и сохранение отчёта в `reports/`;
