...
├── storage.py           # SQLite пользователи, сессии, отчёты и изображения
...
- тарифы Advanced: индивидуальный разовый доступ `$10`, индивидуальная подписка `$25/мес`, индивидуальный годовой доступ `$199/год`, школьный корпоративный тариф `$99/мес` или `$999/год`, тариф для сети школ `$249/мес`;
- серверная проверка Advanced-доступа перед запуском пятиракурсного протокола;
...
Также создаётся аккаунт `admin` с паролем `12345678`. Старые файлы из корневой папки `reports/` мигрируются в SQLite под этим аккаунтом. Тарифы и активные Advanced-подписки тоже хранятся в SQLite.
