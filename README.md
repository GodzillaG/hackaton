...
Сценарий работы: ноутбук запускает сервер анализа и Next.js сайт, телефон находится в той же Wi-Fi сети, открывает сайт по IP ноутбука, проходит протокол съёмки из пяти ракурсов и получает отчёт с уровнем риска, метриками, рекомендациями и изображениями с разметкой.
