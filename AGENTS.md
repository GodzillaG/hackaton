...
Access levels are `Free`, `Plus`, and `Corporate`. Free has limited Basic analyses, Plus has larger Basic quota plus limited monthly Advanced analyses, and Corporate is quote-based for schools with student-status verification. The backend must enforce Basic/Advanced quotas; do not rely only on hiding buttons in the UI. The local demo corporate code is `SCHOOL-ACCESS-2026`.
