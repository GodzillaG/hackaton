...
const ALLOWED_ACTIONS = new Set(["login", "register", "me", "logout"]);
...
async function proxyAuth(request, { params }) {
  ...
  try {
    ...
    const upstream = await fetch(`${PYTHON_API_URL}/api/auth/${action}`, {
      method,
      headers,
    ...
    return Response.json(
      {
        error: `Сервер авторизации недоступен: ${error.message}`
      },
      { status: 503 }
    );
  }
